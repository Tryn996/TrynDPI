...
func print_error(text: Variant, print_godot: = false) -> void :
	if not text is String:
		text = str(text)
	print_line("\t   [color=light_coral]ОШИБКА:[/color] %s" % text, print_godot)


func print_info(text: Variant, print_godot: = false) -> void :
	if not text is String:
		text = str(text)
	print_line("\t   [color=light_blue]ИНФО:[/color] %s" % text, print_godot)


func print_warning(text: Variant, print_godot: = false) -> void :
	if not text is String:
		text = str(text)
	print_line("\t   [color=gold]ПРЕДУПРЕЖДЕНИЕ:[/color] %s" % text, print_godot)
	...
	if not new_text.strip_edges().is_empty():
		...
		if console_commands.has(text_command):
			...
			if (arguments.size() < console_command.required):
				print_error("Слишком мало аргументов! Требуется < %d >" % console_command.required)
				return
			...
			print_error("Команда не найдена.")
...
func exec(filename: String) -> void :
	...
	if (script):
		...
		print_error("Файл %s не найден." % [path])
